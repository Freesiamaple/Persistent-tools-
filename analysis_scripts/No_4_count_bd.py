...
for j in range(len(G)):
    for k in range(len(G[j])):
        if G[j][k]==0 and P[j][k]==0:
            ...
            if G[j][k]==0:
                ax.text(xlim[0]+width*j, ylim[0]+width*k, "o", fontsize=9,c='red')
                continue
            if P[j][k]==0:
                ax.text(xlim[0]+width*j, ylim[0]+width*k, "x", fontsize=9,c='blue')
                continue
            if G[j][k]>P[j][k]:
                ax.text(xlim[0]+width*j, ylim[0]+width*k, str(int(P[j][k]/G[j][k]*100)), fontsize=9,c='blue')
            if G[j][k]<P[j][k]:
                ax.text(xlim[0]+width*j, ylim[0]+width*k, str(int(P[j][k]/G[j][k]*100)), fontsize=9,c='red')
            if G[j][k]==P[j][k]:
                ax.text(xlim[0]+width*j, ylim[0]+width*k,"100", fontsize=9,c='black')
